...
[![](https://img.shields.io/badge/WINDOWS-XP%20%E2%80%93%2011-blue?style=for-the-badge)](#memory-win) [![](https://img.shields.io/badge/SERVER-2003%20%E2%80%93%202025-blue?style=for-the-badge)](#memory-win) [![](https://img.shields.io/github/license/Aditya060806/Memory-Win?color=2ea44f&style=for-the-badge)](/LICENSE) [![](https://img.shields.io/github/downloads/Aditya060806/Memory-Win/total?color=orange&style=for-the-badge)](https://github.com/Aditya060806/Memory-Win/releases/latest)

Memory-Win is a free RAM cleaner that effectively optimizes memory areas by utilizing the native Windows API. This can help improve performance when programs do not properly release allocated memory. Featuring a user-friendly interface and intelligent functionality, this portable application requires no installation; however, it does need administrator privileges to run.

[![](./docs/assets/images/main-window.png)](#memory-win)

## 💾 Download

[![](https://img.shields.io/badge/dynamic/json?url=https%3A%2F%2Fapi.github.com%2Frepos%2FAditya060806%2FMemory-Win%2Freleases%2Flatest&query=%24.tag_name&label=Release&style=for-the-badge)](https://github.com/Aditya060806/Memory-Win/releases/latest/download/MemoryWin.exe)

### 🍫 [Chocolatey](https://community.chocolatey.org/packages/memory-win)

```cmd
choco install memory-win
```

### 🍦 [Scoop](https://scoop.sh/#/apps?q=memory-win&o=true&dm=true&n=true)
...
```cmd
scoop install extras/memory-win
```

### 📦 [WinGet](https://winstall.app/apps/Aditya060806.Memory-Win)

```cmd
winget install Aditya060806.Memory-Win
```
...
4. In MemoryWin, select **only the `Standby List`** and click `Optimize`.
...
```cmd
{path}\MemoryWin.exe /ModifiedFileCache /StandbyList /WorkingSet
```
...
✅ **Install Service:**
```cmd
{path}\MemoryWin.exe /Install
```

❌ **Uninstall Service:**
```cmd
{path}\MemoryWin.exe /Uninstall
```
...
- **Recommended: Use a Package Manager** — If you frequently encounter false positive detections, auto-update issues, or SmartScreen warnings, we **strongly recommend** installing via [Chocolatey](#-chocolatey), [Scoop](#-scoop), or [WinGet](#-winget). These package managers provide:
  ...
  **Example update commands:**
  ...
  ```cmd
  choco upgrade memory-win
  ````
  
  ```cmd
  scoop update memory-win
  ```

  ```cmd
  winget upgrade Aditya060806.Memory-Win
  ```

- If auto-update fails or you need a verified build, download the latest release from the **Download** section above or the Releases page and manually replace your existing `MemoryWin.exe`. Run the new file as administrator.
...
- Simple (non-technical):
  ...
  3. Rename your current `MemoryWin.exe` to `MemoryWin.exe.bak` (backup).
  4. Move the new `MemoryWin.exe` into place and run it as administrator.
  5. If the new version works, delete the `.bak`. To roll back, restore the `.bak` file.

- Advanced (recommended best practices):

  1. Export current settings (registry): `reg export "HKLM\Software\MemoryWin" MemoryWin-reg-backup.reg`
  2. Download the official release and obtain the published SHA256 from the release notes (if provided).
  3. Verify the file hash:

     ```powershell
     Get-FileHash .\MemoryWin.exe -Algorithm SHA256
     ```

-   4. Verify the Authenticode signature:
  
     ```powershell
     Get-AuthenticodeSignature .\MemoryWin.exe
     ```
  ...
  5. Stop running instances (and scheduled task if present) before replacing:
     ...
     ```powershell
     Stop-Process -Name MemoryWin -ErrorAction SilentlyContinue
     ```

  6. Backup and replace safely:

     ```powershell
     Move-Item .\MemoryWin.exe MemoryWin.exe.bak
     Copy-Item C:\path\to\downloaded\MemoryWin.exe .\
     Start-Process .\MemoryWin.exe -Verb RunAs
     ```
...
3. Type (or drag-and-drop) the full path to `MemoryWin.exe`, then type a space and add `/Reset`.

   Example:

   ```cmd
   "C:\Downloads\MemoryWin.exe" /Reset
   ```

   > **💡 TIP:** Drag and drop `MemoryWin.exe` into the Command Prompt — it inserts the full path. If the path contains spaces, enclose it in double quotes before adding ` /Reset`.
...
3. Look for events with the source name **Memory-Win**

[![](./docs/assets/images/windows-event-log.png)](#-logs)

## 🛠️ Complementary Tools

While **Memory-Win** excels at efficiently managing and freeing up memory, the following tools can provide even deeper insights into your system's memory usage and help with advanced troubleshooting:
...
A powerful physical memory usage analysis utility from Microsoft Sysinternals. RAMMap provides detailed, real-time information about how Windows is allocating physical memory, including how much is in use by processes, drivers, the kernel, and various cached data. **It also contains some cleaning features that address certain memory areas similar to Memory-Win.** It's an excellent tool to use alongside MemoryWin to understand precisely *where* memory is being used before and after an optimization.
...
MemoryWin is the **antivirus serum**. It's not a magical cure-all, but a targeted, clean, and honest tool. Here's why it's different and still relevant:
...
They are saved in the Windows registry path `Computer\HKEY_LOCAL_MACHINE\Software\MemoryWin`
...
4. Once tested, please either submit a pull request or submit the file via the **[Translation Request](https://github.com/Aditya060806/Memory-Win/issues/new?template=translation_request.yml)** issue template.
...
3. Rebuild and run the `MemoryWin` project.
...
| 💰 Cash | 🪙 Crypto |
| :--- | :--- |
| [Sponsor on GitHub](https://github.com/sponsors/Aditya060806) | [Bitcoin (BTC)](https://www.blockchain.com/btc/address/YOUR_BTC_ADDRESS_HERE) |
| [Buy me a coffee on Ko-fi](https://ko-fi.com/aditya060806) | [Ethereum (ETH)](https://www.blockchain.com/explorer/addresses/eth/YOUR_ETH_ADDRESS_HERE) |
